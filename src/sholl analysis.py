...
def get_crossing(cur_neuron, step, r_max, custom_soma=None):
    df = cur_neuron.get_segments(custom_soma=custom_soma)
    df['is_cross'] = False
    df['at_cross'] = -1
    r_list = []
    ct_list = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('-X', '--soma_x', type=float, default=None,
                        help='specify soma coordinate: x-axis, use -X "-**" for negative values')
    parser.add_argument('-Y', '--soma_y', type=float, default=None,
                        help='specify soma coordinate: y-axis, use -Y "-**" for negative values')
    parser.add_argument('-Z', '--soma_z', type=float, default=None,
                        help='specify soma coordinate: z-axis, use -Z "-**" for negative values')
